# TDS Format Specification (docx, English)

This file defines the EXACT formatting for generated English TDS Word documents.

## 1. Page Setup
- Page size: Letter (11906 x 16838 twips)
- Margins (twips): top=1361, right=751, bottom=884, left=1785
- Header distance: 688, Footer distance: 716

## 2. Fonts
- Default body font: Arial, 10.5pt (sz=21)
- Table font: Helvetica Neue, 9.5pt (sz=19)
- Header/footer font: 9pt (sz=18)

## 3. Line Spacing
- Body and headings: 1.5 line spacing (w:line=360)
- Table cells: w:line=300

## 4. Header (repeating on every page)
- ONLY two lines, both centered:
  - Line 1: "Shenyang SUN-NOA Technology Development Co., Ltd."
  - Line 2: "Tel: +86-24-25785010 Add：Room 1112, Tianxin Building, No.17A, Jianshe West Road, Tiexi District, Shenyang, China
- Font: 9pt (sz=18)
- No title, no date, no page number in the header

## 5. Footer (repeating on every page)
- Page number (right-aligned, 9pt, sz=18)
- No company logo

## 6. Title Block (body, at the very top)
- Line 1: "Technical Data Sheet" — bold, CENTERED, 14pt (sz=28)
- Line 2: [Product Name] — bold, CENTERED, 15.5pt (sz=31)
- Both lines centered (w:jc="center")
- Spacing before title: 120; spacing before product name: 92

## 7. Section Headings ("Product Description", "Characteristics", etc.)
- Font: Arial, 15.5pt (sz=31), bold
- Spacing before: 93
- Left indent: 21

## 8. Body Text
- Font: Arial, 10.5pt (sz=21)
- Spacing before: 52
- Left indent: 29

## 9. Tables (Technical Specification / Application Parameters)
- Font: Helvetica Neue, 9.5pt (sz=19)
- Borders: none
- Header row: not bold

## 10. Disclaimer (bottom of body)
- Font: Arial, 9pt (sz=18)
- Spacing before: 120
