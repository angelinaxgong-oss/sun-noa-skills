...
## Output Structure (8 sections + disclaimer)
1. Product Description / 产品描述
2. Characteristics / 特点
3. Technical Specification / 技术参数
4. Application Parameters / 施工参数
5. Precautions / 注意事项 — contains subsections such as Safety (安全事项) and Storage (储存); subsection names are flexible
6. Cleaning / 清洁 — OPTIONAL: include ONLY if the source has cleaning info; otherwise OMIT the entire section
7. Shelf Life / 保质期 — ALWAYS leave blank ([To be completed] / [待填写])
8. Packaging / 包装 — ALWAYS leave blank ([To be completed] / [待填写])
...
- Top-level section names are FIXED. CN output must use these exact names in this order: 产品描述 / 特点 / 技术参数 / 施工参数 / 注意事项 / 清洁 / 保质期 / 包装. Always rename source synonyms to these (e.g., 产品规格 → 技术参数, 应用领域 → 产品描述). Subsections INSIDE 注意事项 are flexible (their names follow the source)
...
- Safety (安全事项) and Storage (储存) are subsections INSIDE Precautions (注意事项), NOT separate top-level sections. Their subsection names are flexible and may follow the source wording
- Cleaning (清洁) is OPTIONAL: include only if the source has cleaning info; otherwise omit the entire section
