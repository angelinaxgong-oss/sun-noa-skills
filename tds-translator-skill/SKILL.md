...
- Storage temperature: the storage temperature in the Storage (储存) subsection must be stated as 室温条件 / room temperature. Do NOT copy numeric temperature ranges from the source into the Storage subsection; replace any given temperature with 室温条件 / room temperature.
- Viscosity (unified): express ALL viscosity values in cps with a ± tolerance. 1 mPa·s = 1 cP (cps), so a point value already given as X±Y mPa·s keeps the same number with unit cps. If the source gives a RANGE, convert to midpoint±half-range (e.g., 12,000–18,000 cps → 15000±3000 cps). Always keep the measurement temperature / method in the parameter label (e.g., 粘度（23℃）).
- Pressure (unified): express ALL pressure values in Application Parameters in kg/cm². Conversion factors: 1 MPa = 1 N/mm² ≈ 10.2 kg/cm². Compute and round sensibly (e.g., 0.1–0.8 N/mm² → 1.0–8.2 kg/cm²; 0.2–1.0 MPa → 2.0–10.2 kg/cm²).
- Storage default: if the source provides NO storage conditions at all, insert this fixed instruction in the Storage (储存) subsection:
  CN: 室温条件，避免阳光直射和暴晒雨淋。建议“先进—先用”循环使用。
  EN: Room temperature; protect from direct sunlight, intense sun exposure and rain. A “first in, first out” rotation is recommended.
- Water-based products (1K & 2K): in addition to any source storage conditions, ALWAYS include the following fixed note in the Storage (储存) subsection inside Precautions (注意事项):
  CN: 未使用完的胶，应及时盖紧桶盖，避免表面失水而造成的结皮和外界污染。
  EN: Unused adhesive should be re-sealed promptly to prevent surface skinning caused by moisture loss and external contamination.
